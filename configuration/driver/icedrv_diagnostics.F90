...
!          Cecilia M. Bitz, UW
      ...
      integer (kind=int_kind) :: nt_fbri, nt_Tsfc, nt_fsd, nt_iso
      ...
      call icepack_query_tracer_flags(tr_brine_out=tr_brine,tr_fsd_out=tr_fsd,tr_iso_out=tr_iso)
      call icepack_query_tracer_indices(nt_fbri_out=nt_fbri, nt_Tsfc_out=nt_Tsfc,&
                                        nt_fsd_out=nt_fsd,nt_iso_out=nt_iso)
      ...
      do n = 1, nx
        ...
        do k = 1, n_iso
           work3 (n)  =  (trcr(n,nt_iso  +4*(k-1))*vsno(n) &
                         +trcr(n,nt_iso+1+4*(k-1))*vsno(n) &
                         +trcr(n,nt_iso+2+4*(k-1))*vice(n) &
                         +trcr(n,nt_iso+3+4*(k-1))*vice(n))
...
! author: Elizabeth C. Hunke, LANL
      ...
      integer (kind=int_kind) :: i, k, nt_iso
      ...
      do i = 1, nx
         ...
         pde (i) = work1(i)
         do k = 1, n_iso
            pdiso(i,k) = (trcr(i,nt_iso  +4*(k-1))*vsno(i) &
                         +trcr(i,nt_iso+1+4*(k-1))*vsno(i) &
                         +trcr(i,nt_iso+2+4*(k-1))*vice(i) &
                         +trcr(i,nt_iso+3+4*(k-1))*vice(i))
         enddo
      enddo
