...
!
      ...
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_C2N_in(:)        ! algal C to N (mole/mole)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_chl2N_in(:)      ! 3 algal chlorophyll to N (mg/mmol)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: F_abs_chl_in(:)    ! to scale absorption in Dedd
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_C2N_DON_in(:)    ! increase compare to algal R_Fe2C
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_Si2N_in(:)       ! algal Sil to N (mole/mole)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_S2N_in(:)        ! algal S to N (mole/mole)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_Fe2C_in(:)       ! algal Fe to carbon (umol/mmol)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_Fe2N_in(:)       ! algal Fe to N (umol/mmol)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_Fe2DON_in(:)     ! Fe to N of DON (nmol/umol)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: R_Fe2DOC_in(:)     ! Fe to C of DOC (nmol/umol)

      real (kind=dbl_kind), intent(in), optional :: fr_resp_in         ! frac of algal growth lost due to respiration
      real (kind=dbl_kind), intent(in), optional :: algal_vel_in       ! 0.5 cm/d(m/s) Lavoie 2005  1.5 cm/day
      real (kind=dbl_kind), intent(in), optional :: R_dFe2dust_in      ! g/g (3.5% content) Tagliabue 2009
      real (kind=dbl_kind), intent(in), optional :: dustFe_sol_in      ! solubility fraction
      real (kind=dbl_kind), intent(in), optional :: T_max_in           ! maximum temperature (C)
      real (kind=dbl_kind), intent(in), optional :: op_dep_min_in      ! Light attenuates for optical depths exceeding min
      real (kind=dbl_kind), intent(in), optional :: fr_graze_s_in      ! fraction of grazing spilled or slopped
      real (kind=dbl_kind), intent(in), optional :: fr_graze_e_in      ! fraction of assimilation excreted
      real (kind=dbl_kind), intent(in), optional :: fr_mort2min_in     ! fractionation of mortality to Am
      real (kind=dbl_kind), intent(in), optional :: fr_dFe_in          ! fraction of remineralized nitrogen
                                                           ! (in units of algal iron)
      real (kind=dbl_kind), intent(in), optional :: k_nitrif_in        ! nitrification rate (1/day)
      real (kind=dbl_kind), intent(in), optional :: t_iron_conv_in     ! desorption loss pFe to dFe (day)
      real (kind=dbl_kind), intent(in), optional :: max_loss_in        ! restrict uptake to % of remaining value
      real (kind=dbl_kind), intent(in), optional :: max_dfe_doc1_in    ! max ratio of dFe to saccharides in the ice (nM Fe/muM C)
      real (kind=dbl_kind), intent(in), optional :: fr_resp_s_in       ! DMSPd fraction of respiration loss as DMSPd
      real (kind=dbl_kind), intent(in), optional :: y_sk_DMS_in        ! fraction conversion given high yield
      real (kind=dbl_kind), intent(in), optional :: t_sk_conv_in       ! Stefels conversion time (d)
      real (kind=dbl_kind), intent(in), optional :: t_sk_ox_in         ! DMS oxidation time (d)
      real (kind=dbl_kind), intent(in), optional :: fsal_in            ! salinity limitation factor (1)

      real (kind=dbl_kind), dimension (:), intent(in), optional :: chlabs_in(:)       ! chla absorption 1/m/(mg/m^3)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: alpha2max_low_in(:)  ! light limitation (1/(W/m^2))
      real (kind=dbl_kind), dimension (:), intent(in), optional :: beta2max_in(:)     ! light inhibition (1/(W/m^2))
      real (kind=dbl_kind), dimension (:), intent(in), optional :: mu_max_in(:)       ! maximum growth rate (1/d)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: grow_Tdep_in(:)    ! T dependence of growth (1/C)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: fr_graze_in(:)     ! fraction of algae grazed
      real (kind=dbl_kind), dimension (:), intent(in), optional :: mort_pre_in(:)     ! mortality (1/day)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: mort_Tdep_in(:)    ! T dependence of mortality (1/C)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: k_exude_in(:)      ! algal carbon  exudation rate (1/d)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: K_Nit_in(:)        ! nitrate half saturation (mmol/m^3)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: K_Am_in(:)         ! ammonium half saturation (mmol/m^3)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: K_Sil_in(:)        ! silicon half saturation (mmol/m^3)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: K_Fe_in(:)         ! iron half saturation  or micromol/m^3
      real (kind=dbl_kind), dimension (:), intent(in), optional :: f_don_in(:)        ! fraction of spilled grazing to DON
      real (kind=dbl_kind), dimension (:), intent(in), optional :: kn_bac_in(:)       ! Bacterial degredation of DON (1/d)
      real (kind=dbl_kind), dimension (:), intent(in), optional :: f_don_Am_in(:)     ! fraction of remineralized DON to Am
      real (kind=dbl_kind), dimension (:), intent(in), optional :: f_doc_in(:)        ! fraction of mort_N that goes to each doc pool
      real (kind=dbl_kind), dimension (:), intent(in), optional :: f_exude_in(:)      ! fraction of exuded carbon to each DOC pool
      real (kind=dbl_kind), dimension (:), intent(in), optional :: k_bac_in(:)        ! Bacterial degredation of DOC (1/d)
